import React from 'react';

// 1. Emergency Procedure Air Table (SIL15)
export function EmergencyProcedureAirTable() {
  return (
    <div className="w-full overflow-hidden rounded-lg bg-white">
      <div style={{ marginTop: '-100px', paddingTop: '100px' }}>
        <img
          src="https://cdn.builder.io/api/v1/image/assets%2Fab9db6a681044b748ef518df8cd48de8%2F786bc075ca0445d4b902e3ac2eefb7dc?format=webp"
          alt="Emergency Procedure Air Table (SIL15)"
          className="w-full h-auto block"
        />
      </div>
    </div>
  );
}

// 2. Emergency Procedure 1 for Surface/Ox Table (SOX15) - Irregularity in Decompression
export function EmergencyProcedure1SOX15() {
  return (
    <div className="w-full overflow-hidden rounded-lg bg-white">
      <div style={{ marginTop: '-100px', paddingTop: '100px' }}>
        <img
          src="https://cdn.builder.io/api/v1/image/assets%2Fab9db6a681044b748ef518df8cd48de8%2F4218098246674be3b9464e90a4219161?format=webp"
          alt="Emergency Procedure 1 for Surface/Ox Table (SOX15)"
          className="w-full h-auto block"
        />
      </div>
    </div>
  );
}

// 3. Emergency Procedure 2 for Surface/Ox Table (SOX15) - Oxygen Failure
export function EmergencyProcedure2SOX15() {
  return (
    <div className="w-full overflow-hidden rounded-lg bg-white">
      <div style={{ marginTop: '-100px', paddingTop: '100px' }}>
        <img
          src="https://cdn.builder.io/api/v1/image/assets%2Fab9db6a681044b748ef518df8cd48de8%2F8cfb14cf160640bd83fc4aaa8d04e8bb?format=webp"
          alt="Emergency Procedure 2 for Surface/Ox Table (SOX15)"
          className="w-full h-auto block"
        />
      </div>
    </div>
  );
}

// 4. Emergency Procedure 1 Nitrox Tables - Need for Surface Decompression
export function EmergencyProcedure1NitroxTables() {
  return (
    <div className="w-full overflow-hidden rounded-lg bg-white">
      <div style={{ marginTop: '-100px', paddingTop: '100px' }}>
        <img
          src="https://cdn.builder.io/api/v1/image/assets%2Fab9db6a681044b748ef518df8cd48de8%2F086c02daa6644349a131ec40ad54849b?format=webp"
          alt="Emergency Procedure 1 Nitrox Tables"
          className="w-full h-auto block"
        />
      </div>
    </div>
  );
}

// 5. Emergency Procedure 2 Nitrox Tables - Irregularity in Decompression
export function EmergencyProcedure2NitroxTables() {
  return (
    <div className="w-full overflow-hidden rounded-lg bg-white">
      <div style={{ marginTop: '-100px', paddingTop: '100px' }}>
        <img
          src="https://cdn.builder.io/api/v1/image/assets%2Fab9db6a681044b748ef518df8cd48de8%2Fd767cdc30d3b4b9782372e85acb24477?format=webp"
          alt="Emergency Procedure 2 Nitrox Tables"
          className="w-full h-auto block"
        />
      </div>
    </div>
  );
}

// Backward compatibility
export const CrashDiveProcedure = EmergencyProcedureAirTable;
export const OxygenFailureDuringDecompression = EmergencyProcedure2SOX15;
export const SurfaceDecompressionRequired = EmergencyProcedure1NitroxTables;
export const IrregularityDuringDecompression = EmergencyProcedure1SOX15;
